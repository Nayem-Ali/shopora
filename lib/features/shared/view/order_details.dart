...
class _OrderDetailsState extends State<OrderDetails> {
  ...
  List<String> status = OrderStatus.values.map((e) => e.name).toList();
            ...
            TextButton(
              onPressed: () {
                showDialog(
                  ...
                  builder: (context) {
                    return AlertDialog(
                      title: Text("Are you sure to block this customer from placing order"),
                      actions: [
                        ElevatedButton(
                          onPressed: () async {
              ...
              return Column(
                children: [
                  if (widget.order.status.toLowerCase() != 'cancelled')
                    ProgressTracker(
                      currentIndex: status.indexOf(widget.order.status), // Active step
                      activeColor: Theme.of(context).primaryColor,
                      statusList: [
                        Status(name: "Pending", icon: Icons.access_time),
                        Status(name: "Approved", icon: Icons.check_circle),
